...
const handleMissionUpdate = (newMission) => {
  const missionWithMarkers = {
    ...newMission,
    path: markers.map(marker => marker.getLatLng()) // Save the markers' lat/lng
  };

  if (newMission.id) {
    // Update existing mission
    setMissions(missions.map(mission => 
      mission.id === newMission.id ? { ...missionWithMarkers, date: getCurrentDate() } : mission
    ));
  } else {
    // Add new mission
    setMissions([
